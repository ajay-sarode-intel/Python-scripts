...
#### Arguments Parser ####
parser = argparse.ArgumentParser(description='Script for syn caliber Unclocked registers report')
...
parser.add_argument('--Help', help='Script for finding diff between two reports of syn caliber unclocked registers. Script is format dependent, script will not work if format is changed')
args = parser.parse_args()
old_file_name = args.input_ref
new_file_name = args.input_new
output_file_name = args.output

name_of_sheet = 'UnclockedRegisters'
...
if name_of_sheet not in sheet_names_old_file: 
    sys.exit('UnclockedRegisters Sheet in Reference/Old file does not exists')
...
if name_of_sheet not in sheet_names_new_file: 
    sys.exit('UnclockedRegisters Sheet in New file does not exists')
...
sheet_diff.write(row_of_diff_file, 7, 'Comments',bold_bg_cyan)
sheet_diff.write(row_of_diff_file, 8, 'Waive',bold_bg_cyan)
sheet_diff.write(row_of_diff_file, 9, 'Result',bold_bg_cyan)
row_of_diff_file = row_of_diff_file + 1

##################### Subroutine for conversion of Whole row to Comparable String #######################
col_no_list = [1,2,3,5,6]  # Absolute Columns to be Compared
ref_net_cell_col = 4 # Reference/net/cell/ name column for some part comparison
...
for l1 in range(5,no_rows_new_sheet):
    ...
    for l2 in range(5,no_rows_old_sheet):
        ...
        if(temp_row_new_list==temp_row_old_list): 
            ...
            sheet_diff.write(row_of_diff_file, 9, 'Same as Before',bold_font_blue) # Writing its category in 9th column
...
for l1 in range(5,no_rows_new_sheet):
    ...
    if(x==0):
        ...
        sheet_diff.write(row_of_diff_file, 9, 'New Violation',bold_font_red) # Writing its category in 0th column
        row_of_diff_file = row_of_diff_file + 1
...
for l1 in range(5,no_rows_old_sheet):
    ...
    if(x==0):
        ...
        sheet_diff.write(row_of_diff_file, 9, 'Removed Violation',bold_font_green) # Writing its category in 0th column
        row_of_diff_file = row_of_diff_file + 1
