...
#### Arguments Parser ####
parser = argparse.ArgumentParser(description='Script for syn caliber Logic on Reset report')
...
parser.add_argument('--Help', help='Script for finding diff between two reports of syn caliber logic on reset. Script is strictly format dependent, hence will not work if format is changed')
args = parser.parse_args()
old_file_name = args.input_ref
new_file_name = args.input_new
output_file_name = args.output

name_of_sheet = 'LogicOnResetPathByComb'
...
sheet_diff.write(row_of_diff_file, 8, 'Comments',bold_bg_cyan)
sheet_diff.write(row_of_diff_file, 9, 'Waive',bold_bg_cyan)
sheet_diff.write(row_of_diff_file, 10, 'Result',bold_bg_cyan)
...
for l1 in range(5,no_rows_new_sheet):
    ...
    for l2 in range(5,no_rows_old_sheet):
        ...
        if(temp_row_new_list==temp_row_old_list): 
            ...
            sheet_diff.write(row_of_diff_file, 10, 'Same as Before',bold_font_blue) # Writing its category in 10th column
...
for l1 in range(5,no_rows_new_sheet):
    ...
    if(x==0):
        ...
        sheet_diff.write(row_of_diff_file, 10, 'New Violation',bold_font_red) # Writing its category in 10th column
...
for l1 in range(5,no_rows_old_sheet):
    ...
    if(x==0):
        ...
        sheet_diff.write(row_of_diff_file, 10, 'Removed Violation',bold_font_green) # Writing its category in 10th column
