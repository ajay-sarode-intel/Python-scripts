...
parser = argparse.ArgumentParser(description='Script for wrong gated clk color report')
...
parser.add_argument('--Help', help='Script for finding diff between two reports of wrong gated clk color. Script is format dependent, script will not work if format is changed')
...
wgcc_sheet_old_file = old_file.sheet_by_name('WrongGatedClockColor') 

# Opening new file and its logic on reset sheet for reading 
new_file = xlrd.open_workbook(new_file_name) 
wgcc_sheet_new_file = new_file.sheet_by_name('WrongGatedClockColor') 
...
wgcc_sheet_diff = update_file.add_worksheet('WrongGatedClockColor')

# Add a bold format to use to highlight cells.
bold = update_file.add_format({'bold': 1})

# Number of rows in sheets old and new files
no_rows_old_sheet = wgcc_sheet_old_file.nrows
no_rows_new_sheet = wgcc_sheet_new_file.nrows

# Number of columns in old and new files
no_columns_old_sheet = wgcc_sheet_old_file.ncols
no_columns_new_sheet = wgcc_sheet_new_file.ncols
...
for col_no in range(no_columns_old_sheet):
    wgcc_sheet_diff.write(row_of_diff_file, col_no, str(wgcc_sheet_old_file.col_values(col_no)[4]),cell_format)
row_of_diff_file = row_of_diff_file + 1

# Adding comment line for first section
wgcc_sheet_diff.write(row_of_diff_file, 2, '*** Propagated Violations ***',cell_format2)
row_of_diff_file = row_of_diff_file + 1

col_no_list = [1,2,3,4,5,7,8]
...
for l1 in range(5,no_rows_new_sheet):
    ...
    for m in col_no_list: # making a temporary list of a particular row from new file   
        temp_row_new_list.append(str(wgcc_sheet_new_file.col_values(m)[l1]))

    for l2 in range(5,no_rows_old_sheet):
        temp_row_old_list = []
        for n in col_no_list: # making a temporary list of a particular row from old file 
            temp_row_old_list.append(str(wgcc_sheet_old_file.col_values(n)[l2]))        

        if(temp_row_new_list==temp_row_old_list): 
            
            for col_no in range(1,no_columns_old_sheet):
                wgcc_sheet_diff.write(row_of_diff_file, col_no, str(wgcc_sheet_old_file.col_values(col_no)[l2]))
            wgcc_sheet_diff.write(row_of_diff_file, 6, str(wgcc_sheet_new_file.col_values(6)[l1]))
            row_of_diff_file = row_of_diff_file + 1

# Adding comment line for second section
wgcc_sheet_diff.write(row_of_diff_file,2, '*** New Violations ***',cell_format2)
row_of_diff_file = row_of_diff_file + 1
...
for l1 in range(5,no_rows_new_sheet):
    ...
    if(x==0):
        for col_no in range(1,no_columns_new_sheet):
            wgcc_sheet_diff.write(row_of_diff_file, col_no, str(wgcc_sheet_new_file.col_values(col_no)[l1]))
        row_of_diff_file = row_of_diff_file + 1

# Adding comment line for third section
wgcc_sheet_diff.write(row_of_diff_file,2, '*** Fixed Violations ***',cell_format2)
row_of_diff_file = row_of_diff_file + 1
...
for l1 in range(5,no_rows_old_sheet):
    ...
    for m in col_no_list: # making a temporary list of a particular row from old file   
        temp_row_old_list.append(str(wgcc_sheet_old_file.col_values(m)[l1]))
    x=0
    for l2 in range(5,no_rows_new_sheet):
        temp_row_new_list = []
        for n in col_no_list: # making a temporary list of a particular row from new file 
            temp_row_new_list.append(str(wgcc_sheet_new_file.col_values(n)[l2]))
        
        if(temp_row_old_list==temp_row_new_list): 
            x = 1
    if(x==0):
        for col_no in range(1,no_columns_old_sheet):
            wgcc_sheet_diff.write(row_of_diff_file, col_no, str(wgcc_sheet_old_file.col_values(col_no)[l1]))
        row_of_diff_file = row_of_diff_file + 1
